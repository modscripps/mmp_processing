...
for c=1:length(ig)%length(mmplog(:,1))
    ...
    try
        ...
        [chi, pr_chi, MLEchi]=get_chi1_mmp(drop);
        ...
        MMP.chi(:,c)=10.^nonmoninterp1(pr_chi*100,log10(MLEchi),zgrid); % Edited by ECF
        MMP.w(:,c)=nonmoninterp1(pr_eps*100,w_eps,zgrid);               % 6/28/18 to use MLEchi 
    catch
        disp('problem, continuing')
    end
end
