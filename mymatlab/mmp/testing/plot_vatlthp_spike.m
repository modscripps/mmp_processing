function plot_vatlthp_spike(drop,pr_beg,pr_end,c)
% Function: To aide viewing spikes from raw v1,v2,a1,a2,tl1,tl2,th1,th2,and p data.  
...
%        tl1 and tl2 are subplotted versus pressure and overlaid, in red and black.
%			th1 and th2 are subplotted versus pressure and overlaid, in red and black.
...
% Get raw tl1 and th1 data
tl1=atod1_mmp(read_rawdata_mmp('tl1',drop));
th1=atod1_mmp(read_rawdata_mmp('th1',drop));

% Shorten tl1, th1 to match lengths of interpolated pressure
tl1len=length(tl1);
th1len=length(th1);
tl1=tl1(scanlen/2:tl1len-scanlen/2);
th1=th1(scanlen/2:th1len-scanlen/2);
...
if nargin>1
   ...
   tl1=tl1(ip);th1=th1(ip);
   ipr=find(pr>=p1 & pr<=p2);
end

% Demean a1, a2, tl1, tl2, th1, and th2
a1=a1-(mean(a1(find(~isnan(a1)))));
a2=a2-(mean(a2(find(~isnan(a2)))));
tl1=tl1-(mean(tl1(find(~isnan(tl1)))));
th1=th1-(mean(th1(find(~isnan(th1)))));
...
while strcmp(edit,'y')
   ...
   if strcmp(select,'r')
      ...
      Hp2=plot(p,a2,'-k');
      ylabel('a1(r), a2(b) / V')
      set(Hl2,'xlim',[p1 p2],'layer','top','xgrid','on','xticklabel','');
      
      %%%% Plot tl1 in red
      Hl3=subplot(4,1,3);
      Hp3=plot(p,tl1,'-r');
      ylabel('tl1(r) / V')
      set(Hl3,'xlim',[p1 p2],'layer','top','xgrid','on','xticklabel','',...
         'ylim',[-.02 .02]);     
      ...
      Hp4=plot(p,th1,'-r');
      ylabel('th1(r) / V')
      set(Hl4,'xlim',[p1 p2],'layer','top','xgrid','on','ylim',[-.2 .2]);
      xlabel('p / MPa')
   end
   ...
   if strcmp(select,'z') | strcmp(select,'x')
      ...
      %%%%%%%% Reset and Re-Demean a1 and a2
      a1s=a1-(mean(a1(find(~isnan(a1) & p>=min(xi) & p<=max(xi)))));
      a2s=a2-(mean(a2(find(~isnan(a2) & p>=min(xi) & p<=max(xi)))));
      Hps1=get(Hl2,'children');
      set(Hps1(1),'ydata',a1s,'color','r');
      set(Hps1(2),'ydata',a2s,'color','k');
      
      %%%%%%%% Reset and Re-Demean tl1 and th1
      tl1s=tl1-(mean(tl1(find(~isnan(tl1) & p>=min(xi) & p<=max(xi)))));
      th1s=th1-(mean(th1(find(~isnan(th1) & p>=min(xi) & p<=max(xi)))));
      Hps2=get(Hl3,'children');
      Hps3=get(Hl4,'children');
      set(Hps2(1),'ydata',tl1s,'color','r');
      set(Hps3(1),'ydata',th1s,'color','k');
       
      set(Hl1,'xlim',[min(xi) max(xi)]);
      set(Hl2,'xlim',[min(xi) max(xi)]);
      set(Hl3,'xlim',[min(xi) max(xi)]);
   	set(Hl4,'xlim',[min(xi) max(xi)]);   
   end
   %%%%%%%% end inner loop
