function [Pa1,Pa2,Pv1,Pv2,f]=tlthv1v2spec_mmp_time(drop, sc_lb,sc_ub)
...
istart=sc_lb*HF_PER_SCAN;
istop=sc_ub*HF_PER_SCAN;
...
a1=atod1_mmp(read_rawdata_mmp('tl1', drop));
a1=a1(ispectra);
a2=atod1_mmp(read_rawdata_mmp('th1', drop));
a2=a2(ispectra);
...
title_str=[mmpid ', dr=' int2str(drop) ', ' num2str(sc_lb,4) '-' ...
      num2str(sc_ub,4) ' scanNOs,\fontsize{8} NFFT=' ...
      int2str(NFFT) '/' num2str(length(v1)) '\rm            '];
title(title_str)
ylabel('tl1(red) ,th1(green) / V');
...
title_str=['     ymax=' num2str(yvmax,4)  ' V' ...
...
%minPv=1e-11; maxPv=1e-6; % for no probe test case
minPv=1e-8; maxPv=1e-3; % for higher levels
%keyboard
% Plot v1 spectrum
...
loglog(f,Pa1,'r')
axis([frmin frmax 1e-11 1e-4])
ylabel('Ptl / V ^2 /Hz')
set(ha_a1,'ytick',[10.^ytl_Pa(1:end)]);
