...
if drop_flag==1 | drop_flag==2
  ...
  if isempty(h_tsbe) | isempty(prh_tsbe)
    ...
    axestobelinked = [axestobelinked Hl_w];
  else
     ig=find(~isnan(pbot));
     if ~isempty(ig)
     ...
     dy=.06;
     %
     maxes = multixaxis3(X,Y,xlabeltext,ylabeltext,titletext,[],[],[],[],[],...
        panel_pos(1,:),dy,limits,1,1);
    axestobelinked = [axestobelinked maxes];
  end
  ...
  tiltrange = max(max(tilt(ig,:))) - min(min(tilt(ig,:)));
  tiltmax=max(max(tilt(ig,:))) + 0.2*tiltrange; 
  tiltmin=min(min(tilt(ig,:))) - 0.2*tiltrange; 		
  [m,ntilt]=size(tilt);
  ...
  axestobelinked = [axestobelinked Hl_a1 Hl_a2];
  if ntilt==4
  ...
  if ~isempty(obs) | ~isempty(dox)
     ...
     if ~isempty(dox)
        ...
        if ~isempty(ig)
           ...
           lox = length(dox);
           if lox<ipl_pr(end)
               dox(lox+1:ipl_pr(end))=NaN;
               pr_dox(lox+1:ipl_pr(end))=NaN;
           end
     ...
     if ~isempty(X)
        dy=.06;
        moreaxes = multixaxis3(X,Y,xlabeltext,ylabeltext,titletext,[10 10 10],[],lcolor,[],[],...
           panel_pos(ndone+1,:),dy,limits,1,0);
        axestobelinked = [axestobelinked moreaxes];
        ndone=ndone+1;
     end
  end
  %
