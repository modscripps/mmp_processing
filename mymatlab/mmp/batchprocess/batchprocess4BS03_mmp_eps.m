...
% J. MacKinnon
%modified by DW for Black Sea cruise 03/03, and by MHA for cross-platform 
%compatibility and to incorporate mmp2s, which handles refractometer.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %calc_hofp3G_mmp
   % h_tsbe=[]; prh_tsbe=[]; pbot=[]; hout=[]; pout=[]; last_pbot=[]; % for EPIC01
   ...
   calc_accel1G_mmp % wow, G is set here ONLY
...
%    % 	and pr_vac (pr at ACM); NO corrections for vehicle orientation or motion.
   ...
   drop_flag=1; % recalc, for proper v1,v2 probes in mid-gyre, no 10^-10 min
   calc_eps3G_mmp
   
   if 1==0  % skip chi, plotting for now (sc99) % try again, EPIC01>13461
      calc_tl2_mmp
...
%       end
      ...
      %disp(dstr)
      %pause % to look before continuing during tests
   end
