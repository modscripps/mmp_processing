...
ipr=find(pr_scan>=prstart & pr_scan<=prstop);
...
% Find start and stop sample numbers for a1, a2, v2, v2
...
title_str=[mmpid ', drop=' int2str(drop) ', ' num2str(prstart,4) ' to ' ...
 num2str(prstop,4) ' MPa, NFFT=' int2str(NFFT)];
title(title_str)
...
% Plot v2 spectrum			
axes('position',v2sp_pos,'box','on','yticklabel','','xticklabel','','xticklabel','', ...
     'xscale','log','yscale','log','ticklength',[.03 .025])
hold on
loglog(f,Pv2,'g')
hold on
...
axes('position',a2sp_pos,'box','on','yticklabel','','xticklabel','', ...
     'xticklabel','','xscale','log','yscale','log','ticklength',[.03 .025])
hold on
loglog(f,Pa2,'g')
loglog(f,Pa1,'r')
axis([frmin frmax minPa maxPa])
