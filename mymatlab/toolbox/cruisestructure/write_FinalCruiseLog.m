...
eval(['load ' char(39) cruise_logs '/' cruise '_RawCruiseLog' char(39)])
f = cr;

% New log entries
f.log.script_rawlog = cr.log.script;
f.log.datestr_rawlog = cr.log.datestr;
f.log.script = ['~malford/Data' cruise '\matlab\logs\write_ml_FinalCruiseLog2'];
...
for i = 1:length(get_yday)
    ...
    if strcmp(sys, 'amp')
        eval(['load ' char(39) amp_procdata '\amplog.mat' char(39)])
        logfile = amplog;
...
cr = f;
outfile = fullfile(cr.cruise_logs, [cruise '_FinalCruiseLog']);
